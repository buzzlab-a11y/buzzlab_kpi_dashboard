...
function WeekInput({ month, channel, weekData, onUpdate, listPrice, isMobile }) {
  ...
  const target = useMemo(() => {
    const key = channel === 'instagram' ? 'ig' : channel === 'youtube' ? 'yt' : channel === 'threads' ? 'th' : 'jv';
    const row = ROADMAP_DATA.find(r => r.month === month);
    return row?.[key]?.lists || 0;
  }, [month, channel]);
  ...
  const chipColor = rate >= 100 ? G.success : rate >= 70 ? G.warning : G.error;
    ...
    <Card style={{ padding: isMobile ? 14 : 18 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexWrap: 'wrap', gap: 6 }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {target > 0 && (
            <span style={{ background: chipBg, color: chipColor, borderRadius: G.radiusPill, padding: '2px 10px', fontSize: 11, fontWeight: 600 }}>
              {rate}%
            </span>
          )}
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 4 }}>
            ...
            <div style={{ background: G.border, borderRadius: 4, height: 6, overflow: 'hidden' }}>
              <div style={{
                ...
                background: rate >= 100 ? G.success : theme.main,
...
export default function ChannelDashboard({ channel, inputData, onUpdate }) {
  ...
  const chartData = useMemo(() => ROADMAP_DATA.map(row => {
  ...
  }), [inputData, channel, channelKey, config.listPrice]);

  const totalActualLists = useMemo(() => ROADMAP_DATA.reduce((sum, row) => {
  ...
  const summaryItems = [
    { label: '年間リスト目標', value: totalTargetLists.toLocaleString() },
    { label: '年間売上目標', value: fmtM(totalTargetRevenue) },
    { label: '実績リスト数', value: totalActualLists.toLocaleString() },
      ...
      <Card style={{ padding: isMobile ? '20px' : '24px 28px', overflow: 'hidden', position: 'relative' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, marginTop: 4 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: G.text2 }}>担当: {config.manager} · リスト単価 ¥{config.listPrice.toLocaleString()}</div>
          </div>
        </div>
