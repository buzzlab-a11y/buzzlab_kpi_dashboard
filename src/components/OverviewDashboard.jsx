...
const CHANNELS = ['instagram','threads','youtube'];
...
export default function OverviewDashboard({ inputData, roadmapData, channelAnnual, channelConfigs }) {
  ...
  const pieData = ['instagram','threads','youtube'].map(ch => ({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{
        ...
        <p style={{ fontSize:13,opacity:0.7,marginBottom:24 }}>2026年4月〜12月 · 3チャネル集客戦略</p>
      ...
      <div style={{ display:'grid',gridTemplateColumns:`repeat(${kpiCols},1fr)`,gap:12 }}>
        ...
        />
        {['instagram','threads','youtube'].map(ch => {
          ...
          <table style={{ width:'100%',borderCollapse:'collapse',fontSize:12,minWidth:900 }}>
            <thead>
              <tr style={{ background:G.surfaceVariant }}>
                {['月','IG ACC','IGリスト','IG売上','TH ACC','THリスト','TH売上','YT ACC','YTリスト','YT売上','月間合計','累計売上'].map(h => (
            ...
            <tbody>
              ...
              {(() => {
                ...
                const grandTotal = totals.igRev + totals.thRev + totals.ytRev;
                return (
